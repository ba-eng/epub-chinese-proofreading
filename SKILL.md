...
```bash
python "${CLAUDE_SKILL_DIR}/scripts/proofread.py" pipeline --clean-batches "EPUB文件路径"
```
...
`--clean-batches` 将 `[? ...]` 标记从正文物理剥离，写入独立的 `_checklist.json` 文件。LLM 面对的是干净正文（无法扫标记选段），必须主动扫描每段后再对照 checklist 交叉验证——从根本上消除标记 tunnel-vision。
...
2. **禁止跳过 batch**。全书所有 batch 都必须逐个处理，不得以"术语已经够多"为由跳过后面的 batch。
3. **每个 batch 处理完必须立即 apply-corrections**。后面的 batch 可能发现前面遗漏的术语变体；apply 时 reprocess 会自动把这些新术语传播到全量章节（包括已校对过的章节）。

### 每段操作顺序（防标记 tunnel-vision）

**`[? ...]` 标记是 pipeline 的机械提示，仅作交叉验证。校对的核心驱动是当前轮次的分析维度，而非标记。标记不是待办清单——不能用标记来定位需要处理的段。**

**强制规则——处理全量段，而非标记指向的段：**
- 每个 batch 的全部 segment 必须逐段处理，无论是否有标记
- **禁止扫标记选段**：不允许先扫一遍找 `[?` 再回头只处理有标记的段。正确做法是无视标记、顺序通读，每段都按四步走
- 标记只是步骤 3 的交叉验证——确认自己的判断或发现自己遗漏的东西

处理每个 segment 必须严格按以下顺序：

1. **先读内容（无视标记）**：通读该段全文，理解在上下文中的位置（叙述/对话/内心独白？谁在说话？情绪基调？）。**此步骤不看标记行。**
2. **按当前轮次维度分析**：逐项检查本轮应关注的模式（术语变体 / 翻译腔 / 长句 / 标点等），记下发现。**对每段给出至少一个判断（有问题 / 无问题），不可空过。**
3. **最后看标记**：此时才检查 `[? ...]` 标记。若标记指出的问题已在步骤 2 中发现 → 确认。若标记指出的问题在步骤 2 中未发现 → 重新评估：是标记误报（stale/deliberate choice），还是自己遗漏了？
4. **判断是否修正**：不确定的保留，宁缺毋滥

### 每 batch 自检报告（防作弊）

每个 batch 处理完后、运行 `apply-corrections` 之前，**必须输出**以下自检摘要：

```
**Batch N 自检**
- 总段数：N
- 标记总数：M（其中 [? 英文段落] N 个已自动删除）
- 主动发现（无标记提示、自己扫描发现）：X 个
- 标记确认（标记指出 + 自己也能独立发现）：Y 个
- 标记拒绝（标记误报 / 刻意保留 / stale）：Z 个
- 仅标记发现（标记提醒后才确认、自己最初遗漏）：W 个
```

此报告的用途：
- X > 0 证明做了主动扫描而非纯被动响应标记。如果 X = 0 且 batch 有内容，说明在偷懒
- W > 0 说明标记有价值，但 W 应远小于 X——如果 W 占比过高，说明在偷懒等标记
...
- **第 1 轮全部 batch 处理完后，必须做以下三项全书审计：**
  ...
  **审计 B — Glossary 目标去重**：通读 glossary.json 的全部条目，检查以下两类问题：
  - **自指条目**：`term == target`（如 `"昆提留斯·鲁斯": "昆提留斯·鲁斯"`）。这是无操作死循环，必须修复：将 target 改为正确的 canonical form。**此类条目往往是之前映射链断裂的痕迹——target 未能更新为最新的 canonical form。**
  - **不同 target 指向同一概念**。重点关注：
...
**关键警告——标记 tunnel-vision 在本轮尤为危险：**
- 此时黑名单标记和英文段落标记已全部清除，术语也已统一
- **文本看起来"干净"，但这只是机械层面的干净**——翻译腔、欧化句、标点不规范、角色声音偏离等问题没有标记，只能靠主动逐段扫描
- 前两轮可能习惯了标记驱动的 workflow（看到 `[?` → 处理 → 下一段）。本轮必须彻底切换为**维度驱动**：每段先跑 P1 二维度（翻译腔 7 模式 + 标点），再决定是否检查 P2

**解决方案——第 3 轮机械标记清单：**
`prepare-round3` 命令用正则机械检测 4 类模式并写入 `_checklist.json`，确保 checklist 不为空：
- 翻译腔 7 模式（被……所…… / 是……的 / 一个……的 / 开始……起来 / ……着……着 / 该·其 / 被动过滥(被人们/被大家)）
- 阿拉伯数字（应汉字化）
- 长句 >100 字（欧化断句候选）
- CJK 粘滞（连续相同汉字，逗号插入候选）

LLM 仍必须先逐段主动扫描，再对照 checklist 交叉验证——与第 1-2 轮完全一致的工作流。

```bash
python "${CLAUDE_SKILL_DIR}/scripts/proofread.py" prepare-round3 {work_dir}
```

### 润色范围（区别于纠错）

**五个维度，两层优先级。P1 每段必须逐项判断并产出明确结论；P2 发现才改，不确定的保留。**

**P1 可验证完成标准**——每段至少：
- 翻译腔 7 模式：逐一过目，产出判断（命中 / 不命中）。命中则改写并写入 corrections
- 标点/数字：检查引号层级、省略号、逗号粘滞、阿拉伯数字。有问题则修正
- 两个维度都给了明确判断 ≠ 空过。不可以"看了一眼感觉没问题"就跳过

| 优先级 | 类别 | 具体操作 | 参考标准 |
|--------|------|----------|----------|
| **P1** | 翻译腔消除 | 检测并改写 7 种模式（见下）。每段主动扫描。 | `references/publishing-standards.md` |
| **P1** | 标点/数字规范 | 引号 `""` 双层（内嵌 `''`）。省略号 `……`。逗号防视觉粘滞（跨词边界连续相同 CJK 字符时加逗分隔，如 `之处处处` → `之处，处处`）。数字汉字化。 | `references/publishing-standards.md` |
| **P2** | 欧化长句拆分 | 超过 100 字的单句判断是否断句。排比/意识流/古典口吻 deliberate 长句保留。**只改明显需要断句的。** | |
| **P2** | 角色声音辨识 | 对照 `voice_cards.md`（第 1 轮生成），检查各角色说话风格一致。微调措辞但不改变性格。voice_cards 供参考，不作硬性模板。 | |
| **P2** | 朗读节奏 | 标记拗口长句，提供顺滑建议。保持原文风格。**只改明显拗口的。** | |
...
a) 逐 batch 重新阅读（**标记已全部清除，必须主动扫描。**每段先跑 P1 二维度，再决定是否查 P2。每 batch 完成后输出自检报告）
