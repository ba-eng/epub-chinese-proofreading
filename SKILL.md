...
- `check --glossary` 扫描 HTML/XHTML 中残留的 CJK glossary key——即术语表中有映射但文本中仍未替换的异译名。如果发现残留，说明对应的 `_corrected.json` 段未能覆盖该处文本或 inject 遗漏了该节点，需用 corrections 修补后再重新 inject + pack。
