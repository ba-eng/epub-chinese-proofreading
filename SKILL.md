...
全自动一键校对。用户只需提供 EPUB 路径，全程无需干预，最终获得校对完毕的 EPUB。
...
`pipeline` 自动完成：解包 → 提取正文 → 术语/黑名单机械预处理 → 导出全文（自动按 100K 字分卷 + 相邻卷 2000 字上下文重叠）→ 生成 TASK.md → **自动应用机械修正**（黑名单词替换 + 英文段落删除）。
...
- **跨 batch 人名追踪**：同一个角色名可能在多个 batch 中以不同译名出现。注意**音近字**（如"约书亚/耶苏亚"、"爱卢亚/艾露亚"）、**形近字**、**同音异字**。即使两个译名看起来差异较大，只要它们指向同一外文名或同一角色，就必须统一。
...
- **第 1 轮全部 batch 处理完后，额外做一次全书术语扫描**：快速回看各 batch 的 glossary_additions，检查是否有遗漏的变体（同一外文名可能还有一个你没发现的译法）。
...
> 两轮校对已完成（纠错级，术语+黑名单+英文+翻译腔）。如需进一步提升文学质量，可进入第 3 轮润色（翻译腔深化、欧化句拆分、标点规范、角色声音增强、朗读节奏）。这需要 1M+ 上下文的顶级 LLM。是否继续？
...
| 级别 | 模型 | 润色质量 |
|------|------|----------|
| **推荐** | Claude Opus 4.5+, Claude Sonnet 4.5+ | 能检测翻译腔、拆分欧化句、增强角色声音辨识度 |
| **可尝试** | DeepSeek v4 Pro, Gemini 2.5 Pro | 可做翻译腔检测和标点规范，角色声音和节奏感稍弱 |
| **不推荐** | Haiku 级或更小模型 | 翻译腔判断不稳定，容易过度修改或遗漏 |

**当前会话使用 DeepSeek v4 Pro**：可尝试润色，但建议降低期望——翻译腔 + 标点 + 数字用法的机械性检查能做，角色声音辨识度和朗读节奏可能不够精准。推荐等切换到 Opus/Sonnet 4.5+ 时再做。
...
| 欧化长句拆分 | 超过 100 字的单句，判断是否需要断句 | 保持原意，不强制拆分 |
| 标点/数字规范 | 引号层级 `「」『』`、省略号 `……`、数字汉字化 | `references/publishing-standards.md` |
| 角色声音辨识 | 对照 `voice_cards.md`（第 1 轮生成的角色对话样本），检查各角色说话风格是否一致（文雅/粗鄙/简洁/啰嗦），辨识度低的微调措辞使角色声音更鲜明 | 不改变性格和情节。voice_cards 样本少（每角色 3-4 句），仅作风格参考，不作为硬性模板 |
