...
- 前两轮可能习惯了标记驱动的 workflow（看到 `[?` → 处理 → 下一段）。本轮必须彻底切换为**维度驱动**：每段先跑 P1 二维度（翻译腔 4 模式 + 标点），再决定是否检查 P2

**解决方案——第 3 轮机械标记清单：**
`prepare-round3` 命令用正则机械检测 4 类模式并写入 `_checklist.json`，确保 checklist 不为空：
- 翻译腔 4 模式（被……所…… / 开始……起来 / 该·其 / 被动过滥(被人们/被大家)）
- 阿拉伯数字（应汉字化）
- 长句 >100 字（欧化断句候选）
...
**P1 可验证完成标准**——每段至少：
- 翻译腔 4 模式：逐一过目，产出判断（命中 / 不命中）。命中则改写并写入 corrections
- 标点/数字：检查引号层级、省略号、逗号粘滞、阿拉伯数字。有问题则修正
- 两个维度都给了明确判断 ≠ 空过。不可以"看了一眼感觉没问题"就跳过

| 优先级 | 类别 | 具体操作 | 参考标准 |
|--------|------|----------|----------|
| **P1** | 翻译腔消除 | 检测并改写 4 种模式（见下）。每段主动扫描。 | `references/publishing-standards.md` |
