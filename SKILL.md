...
- `check --glossary` 必须在 `pack` 之后运行。`pack` 会做最后一层 glossary 写回，pack 后再检查才能验证最终 EPUB 工作目录中的术语覆盖。如果发现残留，说明对应的 `_corrected.json` 段未能覆盖该处文本或 inject/pack 遗漏了该节点，需用 corrections 修补后再重新 inject + pack + check --glossary。
- `check` 返回的非零退出码通常是英文删除导致的 change ratio 超阈值，属于预期行为。不要用 `&&` 串联最终命令；如果 diff 报告中修改段数与你的 corrections 数量吻合，可以继续 inject + pack + check --glossary。
...
d) 全部 batch 完成后：`check --diff` → `inject` → `pack` → `check --glossary`
